            ...
            if (analysis.red_markups_list && analysis.red_markups_list.length > 0) {{
                redMarkupsHTML = analysis.red_markups_list.map(red => `
                    <div class="change-item severity-high">
                        <div class="change-type">📍 ${{red.grid_position}} • ${{red.keyword}}</div>
                ...
                unresolvedHTML = `
                    <div class="result-card">
                        ...
                        ${{analysis.unresolved_items.map(item => `
                            <div class="change-item severity-high">
                                <div class="change-type">${{item.position}}</div>
                                <div class="change-desc">${{item.comment}}</div>
                            </div>
                        `).join('')}}
                        ${{comparison.status === 'NONE_RESOLVED' ? '<p style="color: var(--secondary); font-weight: bold; margin-top: 20px; padding: 20px; background: rgba(255,0,110,0.1); border-radius: 10px;">❌ NO CHANGES DETECTED/UPDATED PROPERLY<br><br>Please redo/recheck the PDF attached.<br><br>Manual check needed!</p>' : ''}}
                    </div>
                `;
            }}
